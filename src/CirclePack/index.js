...
class CirclePack extends AbstractChart {
  constructor(canvasId, _userOptions) {
    ...
    this.barDimVal = d => {
      return d[this._options.data.x.accessor];
    };
    this.barMetVal = d => {
      return d[this._options.data.y.accessor];
    };
  ...
  render(_data) {
    ...
    this.diameter = Math.min(
      this._options.chart.innerWidth * 0.9,
      this._options.chart.innerHeight * 0.9
    );
    ...
    this._hierarchy = hierarchy(layoutData, d => {
      return d.children;
    }).sum(d => {
      return d.size;
    });
    ...
    document
      .getElementById(this._fontCanvasId)
        ...
        if (node !== nodeOld) {
          ...
          if (node) {
            ...
            if (has(node.data, '_data')) {
              ...
              let nodeX =
                  (node.x - this.zoomInfo.centerX) * this.zoomInfo.scale +
                  this.centerX,
                nodeY =
                  (node.y - this.zoomInfo.centerY) * this.zoomInfo.scale +
                  this.centerY,
              ...
              document
                .getElementById(this._containerId.substring(1))
                .appendChild(div);
  ...
  _drawCanvas(chosenContext, hidden = false) {
    ...
    for (let i = 0; i < this.nodeCount; i++) {
      ...
      let nodeX =
          (node.x - this.zoomInfo.centerX) * this.zoomInfo.scale + this.centerX,
        nodeY =
          (node.y - this.zoomInfo.centerY) * this.zoomInfo.scale + this.centerY,
        nodeR = node.r * this.zoomInfo.scale;
      ...
      if (has(node.data, '_data')) {
        ...
        if (sameBranch(node, this.focus) && !hidden) {
          ...
          if (drawTitle & this.showText) {
            ...
            chosenContext.font =
              ...
              'px ' +
              this._options.bodyFont;
            chosenContext.fillStyle =
              'rgba(191,191,191,' + this.textAlpha + ')'; //"#BFBFBF";
            chosenContext.textAlign = 'center';
            chosenContext.textBaseline = 'middle';
            chosenContext.fillText(
              this._options.plots.titleFn(commaFormat(node.data.size)),
              nodeX,
              nodeY + -0.75 * nodeR
            );

            //Get the text back in pieces that will fit inside the node
            let titleText = getLines(
              chosenContext,
              node.data.name,
              nodeR * 2 * 0.7,
              fontSizeTitle,
              this._options.titleFont
            );
            //Loop over all the pieces and draw each line
            titleText.forEach((txt, iterator) => {
              chosenContext.font =
                fontSizeTitle + 'px ' + this._options.titleFont;
              chosenContext.fillStyle =
                'rgba(' +
                this._options.plots.mainTextColor[0] +
                ',' +
                this._options.plots.mainTextColor[1] +
                ',' +
                this._options.plots.mainTextColor[2] +
                ',' +
                this.textAlpha +
                ')';
              chosenContext.textAlign = 'center';
    ...
    for (let i = 0; i < this.nodeCount; i++) {
      ...
      if (!isUndefined(node.parent) && !isUndefined(node.children)) {
        if (
          (node.data.name !== this.rootName) &
          !hidden &
          this.showText &
          (includes(this.kids, node.data.name) >= 0)
        ) {
  ...
  _drawBars(chosenContext, node, nodeX, nodeY, nodeR) {
    ...
    let eachBarHeight =
      (1 - this._options.plots.barChartHeightOffset) *
      2 *
      nodeR *
    ...
    if (Math.round(barHeight) > 1) {
      ...
      for (let j = 0; j < bars.length; j++) {
        ...
        bar.width = isNaN(this.barMetVal(bar))
          ? 0
          : barScale(this.barMetVal(bar));
        bar.barPiecePosition =
          nodeY +
          this._options.plots.barChartHeightOffset * 2 * nodeR +
          j * eachBarHeight -
          this._options.plots.barChartHeight * nodeR;
        ...
        if (drawValueText & this.showText) {
          ...
          chosenContext.fillStyle =
            valuePos === 'left'
              ? 'rgba(51,51,51,' + this.textAlpha + ')'
              : 'rgba(255,255,255,' + this.textAlpha + ')'; //#333333 or white
  ...
  _interpolateFadeText(dt) {
    if (this.fadeText) {
      this.timeElapsed += dt;
      this.textAlpha = this._cubicEase(
        this.timeElapsed / this.fadeTextDuration
      );
