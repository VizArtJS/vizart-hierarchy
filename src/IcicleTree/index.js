...
class IcicleTree extends AbstractChart {
  ...
  render(_data) {
    ...
    let xScale = scaleLinear().range([0, this._options.chart.innerWidth]);

    let yScale = scaleLinear().range([0, this._options.chart.innerHeight]);
    ...
    let _tree = hierarchy(this._data).sum(d => {
      return d.size;
    });
