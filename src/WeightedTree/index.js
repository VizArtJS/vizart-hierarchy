...
class WeightedTree extends AbstractChart {
  constructor(canvasId, _userOptions) {
    ...
    this.nodeRadius = node => {
      ...
      this.nodeScale.domain([
        this.minValues[node.depth],
        this.maxValues[node.depth],
      ]);
  ...
  layoutTree() {
    ...
    if (this._options.plots.branchPadding == -1) {
      ...
      scale =
        Math.min(
          this._options.chart.innerHeight,
          this._options.chart.innerWidth
        ) * this._options.plots.branchPadding;
    }
    ...
    this.depthSpan =
      this._options.plots.fixedSpan > 0
        ? this._options.plots.fixedSpan
        : this._options.width / (this.maxDepth + 1);

    //Set max/min values
    for (let i = 1; i < this.maxDepth + 1; i++) {
      let vals = this.root.descendants().filter(d => {
        return d.depth === i;
      });
      this.maxValues[i] = max(vals, d => {
        return d.value;
      });
      this.minValues[i] = min(vals, d => {
        return d.value;
      });
    }
  }
  ...
  update() {
    ...
    this.node
      ...
      .attr('x', d => {
        return d.children
          ? -this._options.plots.textOffset
          : this._options.plots.textOffset;
      })
      .style('text-anchor', d => {
  ...
  mode(_mode) {
    ...
    this.node.transition(t).attr('transform', d => {
      return 'translate(' + d.y + ',' + d.x + ')';
    });
